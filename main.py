...
import argparse # manipulação de argumento
import cv2 #opencv
import numpy as np # biblioteca que facilita a manipulação/operações de arrays e matrizes 
import matplotlib.pyplot as plt # biblioteca para manipulação de imagens
from dataset import create_dataset
import math # biblioteca matematica


def parse_argument():# passar o caminho dos arquivos 
...
def main():
    ...
    for index in range(dataset.image_count):
        ...
        points = np.float32(list(map(lambda x: [x.pt], prev_keypoint)))
