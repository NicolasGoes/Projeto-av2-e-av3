...
class CameraParameters(): # Declarando uma classe 
    def __init__(self, fx, fy, cx, cy): # construtor da classe 
        self.fx = fx
        self.fy = fy
        self.cx = cx
        self.cy = cy
    ...
    @property
    def camera_matrix(self): # declarando uma função
...
class Dataset(): #declarando uma classe para tratar os dados
    def __init__(self):
        pass

    #os.path.join -> unir o caminho do arquivo
    def image_path_left(self, index):#declarando uma função do camimho
    ...
    def load_ground_truth_pose(self, gt_path):
        ...
        with open(gt_path) as gt_file: # percorrer as linhas do arquivo
            gt_lines = gt_file.readlines()

            for gt_line in gt_lines:
                pose = self.convert_text_to_ground_truth(gt_line)
                ground_truth.append(pose)  # inserir no array
...
class KittiDataset(Dataset):
    def __init__(self, path):
        ...
        gt_path = os.path.join(self.path, sequence_count + '.txt')
